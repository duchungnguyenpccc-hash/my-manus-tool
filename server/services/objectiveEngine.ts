...
function extractPatterns(text?: string) {
  ...
  for (let index = 0; index < tokens.length - 1; index += 1) {
    patterns.add(`${tokens[index]} ${tokens[index + 1]}`);
  }
...
function inferContentStyle(profile: OptimizationProfileState): ProductionPolicy["contentStyle"] {
  if (profile.performanceBaseline.averageObjectiveScore >= 1.25 && profile.wins > profile.losses) {
    return "aggressive";
  }
  if (profile.performanceBaseline.averageObjectiveScore < 0.85) {
    return "experimental";
  }
  return "balanced";
}
...
export const objectiveEngine = {
  ...
  async getProfile(userId: number, nicheId: number): Promise<OptimizationProfileState> {
    ...
    const rows = await db
      ...
      .where(and(eq(optimizationProfiles.userId, userId), eq(optimizationProfiles.nicheId, nicheId)))
      .limit(1);
  ...
  evaluateObjective(input: { metrics: ObjectiveOutcomeInput["metrics"]; baseline: PerformanceBaseline; objectiveWeights?: ObjectiveWeights }) {
    ...
    const objectiveScore =
      viewLift * weights.views +
      watchLift * weights.watchTime +
      revenueLift * weights.revenue;
  ...
  updatePatternScores(scoreMap: Record<string, number>, text: string | undefined, delta: number) {
    for (const pattern of extractPatterns(text)) {
      scoreMap[pattern] = Number(clamp((scoreMap[pattern] ?? 0) + delta, -5, 10).toFixed(3));
    }
    return scoreMap;
  },
  ...
  updateBlacklist(profile: OptimizationProfileState, topic: string, won: boolean) {
    ...
    for (const [key, value] of Object.entries(profile.hookPatternScores)) {
      if (key.startsWith("__kill__:")) penalties[key.replace("__kill__:", "")] = value;
    }

    for (const pattern of extractPatterns(topic)) {
      penalties[pattern] = (penalties[pattern] ?? 0) + (won ? -1 : 1.25);
    }

    for (const [pattern, value] of Object.entries(penalties)) {
      profile.hookPatternScores[`__kill__:${pattern}`] = Number(clamp(value, 0, 5).toFixed(3));
    }

    profile.blacklistedPatterns = Object.entries(penalties)
  ...
  async recordOutcome(input: ObjectiveOutcomeInput) {
    ...
    const evaluation = this.evaluateObjective({
      metrics: input.metrics,
      baseline: profile.performanceBaseline,
      objectiveWeights: profile.objectiveWeights,
    });
